export default class SAudio extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!videoId) {
      this.innerHTML = `<p>Video no válido</p>`;
      return;
    }
    ...
    this.innerHTML = `
      <style>
        ...
        s-audio .s-audio-title {
          font: var(--f-m) var(--f-display-sans);
        }
      ...
      <article class="s-audio">
        ...
        <iframe
          ...
          src="https://www.youtube.com/embed/${videoId}?enablejsapi=1&origin=${origin}&playsinline=1&controls=0"
        ></iframe>
      </article>
    `;

    this.button =
      this.querySelector(".s-audio-button");

    this.status =
      this.querySelector(".s-audio-status");
  ...
  toggle() {
    ...
    if (!this.playing) {
      ...
      this.button.setAttribute("aria-label", "Pausar audio");
      ...
      this.playing = false;
      this.button.innerHTML = "▶";
      this.button.setAttribute("aria-label", "Reproducir audio");
      this.status.innerHTML = "detenido";
    }
  }
