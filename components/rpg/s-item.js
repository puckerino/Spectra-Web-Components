export default class SItem extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    this.render(item, cantidad, status);
  }
  ...
  waitForItems(itemsKey) {
    return new Promise(resolve => {
      ...
      window.addEventListener(
        "spectra:items-ready",
        () => {
          resolve(Array.isArray(window[itemsKey]));
        },
        { once: true }
      );
      ...
      <article class="s-item-inner">
        ...
        <header class="s-item-header">
          ...
          <div class="s-item-meta">
            ${categoria ? `<span class="s-item-chip">${categoria}</span>` : ""}
            ${tipo ? `<span class="s-item-chip">${tipo}</span>` : ""}
            ${cantidad ? `<span class="s-item-chip">x${cantidad}</span>` : ""}

            ${
              status === "activo"
                ? `<span class="s-item-chip s-item-status is-active">activo</span>`
                : ""
            }

            ${
              status === "agotado"
                ? `<span class="s-item-chip s-item-status is-empty">agotado</span>`
                : ""
            }
          </div>
        </header>
